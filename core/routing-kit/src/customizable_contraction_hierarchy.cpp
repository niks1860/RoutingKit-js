...
namespace RoutingKit
{
	...
	namespace
	{
		...
		struct TriangleVerifier
		{
			...
			bool operator()(
					...
					unsigned bottom_node, unsigned mid_node, unsigned top_node) const
			{
	...
	CustomizableContractionHierarchy::CustomizableContractionHierarchy(
			...
			bool filter_always_inf_arcs) : order(std::move(arg_order))
	{
	...
	CustomizableContractionHierarchyMetric::CustomizableContractionHierarchyMetric(const CustomizableContractionHierarchy &cch, const std::vector<unsigned> &input_weight) : forward(cch.cch_arc_count()), backward(cch.cch_arc_count()), cch(&cch), input_weight(&input_weight[0])
	{
	}

	CustomizableContractionHierarchyMetric::CustomizableContractionHierarchyMetric(const CustomizableContractionHierarchy &cch, const unsigned *input_weight) : forward(cch.cch_arc_count()), backward(cch.cch_arc_count()), cch(&cch), input_weight(input_weight)
	{
	}
	...
	CustomizableContractionHierarchyParallelization::CustomizableContractionHierarchyParallelization(const CustomizableContractionHierarchy &cch)
	{
		...
		auto p = compute_stable_sort_permutation_using_key(node_level, level_count, [&](unsigned x)
																											 { return x; });
...
#pragma omp for schedule(dynamic, 256)
	...
	CustomizableContractionHierarchyPartialCustomization::CustomizableContractionHierarchyPartialCustomization(const CustomizableContractionHierarchy &cch_) : q(cch_.cch_arc_count()),
																																																																														 cch(&cch_)
	{
	}
	...
	CustomizableContractionHierarchyQuery::CustomizableContractionHierarchyQuery(const CustomizableContractionHierarchyMetric &metric) : forward_tentative_distance(metric.cch->node_count(), inf_weight),
			...
			forall_ancestors(
					...
					[&](unsigned x)
					{
						relax_outgoing_arcs(
								...
								forward_tentative_distance, [&](unsigned a, unsigned b)
								{ forward_predecessor_node[a] = b; },
								x);
						return true;
					});
		}
			...
			forall_ancestors(
					...
					[&](unsigned x)
					{
						relax_outgoing_arcs(
								...
								backward_tentative_distance, [&](unsigned a, unsigned b)
								{ backward_predecessor_node[a] = b; },
								x);
						if (in_forward_search_space[x])
						{
	...
	namespace
	{
		...
		{
			TriangleUnpacker() {}
			TriangleUnpacker(unsigned &bottom_node, unsigned &bottom_arc, unsigned &mid_arc) : bottom_node_(&bottom_node), bottom_arc_(&bottom_arc), mid_arc_(&mid_arc) {}
			bool operator()(
					unsigned bottom_arc, unsigned mid_arc, unsigned top_arc,
					unsigned bottom_node, unsigned mid_node, unsigned top_node) const
			{
